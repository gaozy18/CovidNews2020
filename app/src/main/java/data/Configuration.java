...
public class Configuration {
    ...
    private void loadConfig(){
        available_categories = new ArrayList<Integer>();
        for (int x = 0;x < Constant.CATEGERIES_COUNT;x++)
        {
            available_categories.add(x);
        }
    };//载入用户设置
    ...
    */
    ...
    {
        List<Category> list = new ArrayList<>();
        for (int x = 1;x < Constant.CATEGERIES_COUNT;x++)
        {
            if (!available_categories.contains(x)) list.add(new Category(Constant.CATEGERIES[x],x));
        }
        return list;
    }
    /*
    ...
    public boolean addCategory(Category category)
    {
        if (!available_categories.contains(category.idx)){
            available_categories.add((Integer) category.idx);
            saveConfig();
            return true;
        }
        return false;
